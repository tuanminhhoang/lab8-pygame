...
class Boid:
    def __init__(self) -> None:
        ...
        self.center = Vector2(self.x + Config.BOID_SIZE / 2, self.y + Config.BOID_SIZE / 2)
    ...
    def _separation(self, boids: List['Boid']) -> pygame.Vector2:
        distances = [['diagonal', 1000]]
        for boid in boids: 
            distance = self.center.distance_squared_to(boid.center)
            if distance < distances[-1][1]:
                distances.append(boid, distance)
        closest = distances[-1][0]
        if distances[-1][1] > Config.SEPARATION_DISTANCE:
            v = self.center - closest.center
            if v.length_squared() == 0:
                return Vector2(0, 0)
            v = v.normalize()
            v *= Config.SEPARATION_STEER_STRENGTH
            return v
